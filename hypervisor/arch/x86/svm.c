...
int svm_cell_init(struct cell *cell)
{
	...
	cell->svm.iopm[I8042_CMD_REG / 8] |= 1 << (I8042_CMD_REG % 8);
				 ...
				 unsigned long *gpr)
{
	...
	guest_page = page_map_get_guest_pages(pg_structs, rip,
...
void svm_cpu_park(struct per_cpu *cpu_data)
{
	...
	err = page_map_create(&hv_paging_structs,
			arch_page_map_gphys2phys(cpu_data,
				target_addr, PAGE_DEFAULT_FLAGS),
